...
# تنظیمات
TOKEN = "8283541397:AAEd23YdmibrDKwxoX0SyhiF6vuywwxnSPk"  # توکن ربات خود را اینجا قرار دهید
if not TOKEN or TOKEN == "YOUR_BOT_TOKEN_HERE":
    raise ValueError("لطفاً توکن ربات خود را در متغیر TOKEN قرار دهید")

ADMIN_IDS = [7770582235, 7866822844]  # آیدی مدیران
...
سپس عکس رسید همراه با نام و نام خانوادگی خود را ارسال کنید."""
...
📱 کد محصول: {PRODUCTS['site_normal']['code']}
    ...
    # iOS محصولات
        ...
        try:
            await query.edit_message_text("🎮 چیت آیفون:\n\nدرحال اپدیت", reply_markup=InlineKeyboardMarkup(keyboard))
        except Exception:
            await query.message.reply_text("🎮 چیت آیفون:\n\nدرحال اپدیت", reply_markup=InlineKeyboardMarkup(keyboard))
...
💳 روش پرداخت: کارت به کارت"""
    ...
    # پردازش رسیدها (عکس + متن)
    if user_state.get('waiting_for_receipt') or update.message.photo:
        if not user_state.get('waiting_for_receipt'):
            return
...
"""
    ...
    if user_state.get('waiting_for_charge_amount'):
        try:
            ...
            # ذخیره مبلغ و انتقال مستقیم به کارت به کارت
            user_states[user_id] = {
                'charge_amount': amount
            }
...
سپس عکس رسید همراه با آیدی خود ({user_id}) و مبلغ را ارسال کنید."""
    ...
    if user_state.get('waiting_for_charge_receipt'):
        ...
        if not update.message.photo:
            await update.message.reply_text(
                "❌ باید عکس رسید با نام و نام خانوادگی ارسال کنید",
                reply_markup=main_menu(user_id)
            )
            return
            
        # دریافت photo file_id
        try:
            photo_file_id = update.message.photo[-1].file_id
            logger.info(f"عکس رسید شارژ دریافت شد - User: {user_id}, Photo ID: {photo_file_id}")
        except (IndexError, AttributeError) as e:
            logger.error(f"خطا در دریافت file_id عکس: {e}")
            await update.message.reply_text(
                "❌ خطا در پردازش عکس. لطفاً دوباره ارسال کنید.",
                reply_markup=main_menu(user_id)
            )
            return
        ...
        for admin_id in ADMIN_IDS:
            try:
                await context.bot.send_photo(
                    admin_id,
                    photo_file_id,
                    caption=charge_receipt_text,
                    reply_markup=InlineKeyboardMarkup(keyboard)
                )
                sent_count += 1
                logger.info(f"رسید شارژ با موفقیت برای ادمین {admin_id} ارسال شد")
            except Exception as e:
                logger.error(f"خطا در ارسال عکس رسید شارژ به ادمین {admin_id}: {e}")
                # در صورت خطا در ارسال عکس، متن ارسال کن
                try:
                    await context.bot.send_message(
                        admin_id,
                        charge_receipt_text + "\n\n⚠️ خطا در ارسال عکس!",
                    ...
                    logger.info(f"متن رسید شارژ برای ادمین {admin_id} ارسال شد (بدون عکس)")
                except Exception as e2:
                    logger.error(f"خطا در ارسال متن رسید شارژ به ادمین {admin_id}: {e2}")
        
        logger.info(f"رسید شارژ برای {sent_count}/{len(ADMIN_IDS)} ادمین ارسال شد")
        
        # پیام تایید برای کاربر
        await update.message.reply_text(
            "✅ درخواست شارژ شما دریافت شد!\n\nبزودی مبلغ واریزی به حساب کاربرتون اضافه میشود!",
            reply_markup=main_menu(user_id)
        )
